...
def get_nn(i1, data, values):
    ...
    for i in range(data.shape[0]):
        if random.randint(0, 8) not in (0, 1):
    ...
    model = keras.Sequential()
    model.add(keras.layers.Convolution2D(32, (2, 2),  input_shape=(32, 32, 1), activation='relu', padding='same'))
    ...
    model.fit(data_train, val_train, epochs=30)
...
for i in range(data.shape[0]):
    if random.randint(0, 8) not in (0, 4):
        data_train.append(data[i])
